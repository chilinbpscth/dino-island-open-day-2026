...
- `npm test`：46 項單元測試通過，涵蓋計分、後台計時、媒體生命週期、指令、姿態判定與描寫；新增驗收頁使用實際 Apps Script 原始碼的本機測試，包含已有玩家／Top 10 已滿，並區分連線錯誤與時間驗證錯誤。
...
- 本輪本機建置包含 156 項離線資源，版本 `6933cd37c9b3`；46 項單元測試、十科完整流程、離線流程及本輪人文／科學專項測試通過。建置會清除輸出資料夾舊檔，避免撤下的錯誤圖留在部署套件。公開版本以 GitHub Pages workflow 結果及線上 manifest 為準。
