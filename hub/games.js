...
import {mountMotion} from './modules/motion.js';
import {mountScience} from './modules/science.js';
...
export function mountGame(root,zone,state,save,finish,lifecycle={}){
 ...
 const cleanupView=()=>{stopVoice();timers.forEach(clearTimeout);timers=[];listeners.forEach(f=>f());listeners=[];cancelAnimation();ghost?.remove();ghost=null;currentAudio?.pause();};
 ...
 on(root.querySelector('#character-listen'),'click',()=>playVoice(new URL(`./audio/yue/characters/${['mountain','water','fire','wood','sun','moon'][state.round]}.wav`,import.meta.url)).then(ok=>{if(!ok)feedback('字音暫時未能播放，可以跟筆順繼續。');}));on(root.querySelector('#stroke-demo'),'click',()=>demo());on(root.querySelector('#stroke-help'),'click',()=>{svg.classList.add('trace-assisted');demo(4000);feedback('同小蓮一齊，由綠點跟住金線慢慢寫，仍然要自己落筆。');});demo();
