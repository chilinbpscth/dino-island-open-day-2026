...
export function mountHumanities(root,state,save,finish){
 ...
 function render(){
  ...
  root.innerHTML=`<div class="rounds" aria-label="第 ${state.round+1} 回合，共三回合">${tasks.map((_,i)=>`<span class="round-dot ${i<=state.round?'done':''}"></span>`).join('')}</div><h2 class="game-instruction">時光探險營地</h2>${!state.started?`<div class="expedition-intro">${picture('machine')}<p>一齊玩時光機想像遊戲！<br>去恐龍島研究，幫小龍搵安全嘅生活地方。</p><button class="primary" id="expedition-start">出發去研究</button></div>`:`<p class="game-instruction">${task.title}</p><div class="expedition-camp ${arriving?'is-arriving':''}"><img class="character" src="img/chars/xiaolian-point.png" alt="小蓮帶領探險"><div class="expedition-improvements" aria-label="營地照顧成果">${achieved>=1?`<span class="${state.roundDone&&achieved===1?'camp-added':''}">${picture('camp')}安全營地</span>`:''}${achieved>=2?`<span class="${state.roundDone&&achieved===2?'camp-added':''}">${picture('bottle')}清潔飲用水</span>`:''}${achieved>=3?`<span class="${state.roundDone&&achieved===3?'camp-added':''}">${picture('forest')}樹蔭</span>`:''}</div><img class="character ${state.roundDone?'is-happy':''}" src="img/chars/dino-baby.png" alt="營地小龍"></div><div class="choices expedition-options">${state.roundDone?'':task.options.map(([id,label])=>`<button class="choice" data-expedition="${id}" aria-label="${label}">${optionPicture(id)}<span>${label}</span></button>`).join('')}</div><p class="feedback" role="status">${state.roundDone?task.reward:state.attempts>=2?task.hint:'望清楚環境，揀一樣幫小龍。'}</p><div class="actions">${!state.roundDone&&hintLevel(state.attempts)==='guided'?'<button id="expedition-help">一齊搵安全方法</button>':''}${state.roundDone?`<button class="primary" id="expedition-next">${state.round===2?'完成照顧任務':'繼續建好營地'}</button>`:''}</div>`}`;
