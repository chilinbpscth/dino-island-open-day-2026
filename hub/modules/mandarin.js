...
export function mountMandarin(root,state,save,finish,{pause=()=>{},resume=()=>{}}={}){
 ...
 function render(){
  clearAudio();const [id,phrase,prompt]=greetings[state.round];const guide=state.round%2===0?'xiaolian':'xiaozhi',guideName=state.round%2===0?'小蓮':'小志';
  const pose=id==='thanks'?'thanks':id==='goodbye'?'goodbye':'greeting';
  const setPose=active=>{if(!live)return;const image=root.querySelector('.greeting-village>.dino');image.src=`img/games/mandarin/dino-${active}.png`;image.alt=active==='listening'?'小龍專心聽你講':active==='thanks'?'小龍收到水果，開心多謝':active==='goodbye'?'小龍揮手講再見':'小龍打招呼';};
  root.innerHTML=`<div class="rounds" aria-label="第 ${state.round+1} 句，共六句">${greetings.map((_,i)=>`<span class="round-dot ${i<=state.round?'done':''}"></span>`).join('')}</div><h2 class="game-instruction">恐龍打招呼</h2><div class="greeting-content"><div class="stage greeting-village"><img class="greeting-backdrop" src="img/games/mandarin/village.png" alt="恐龍村：朋友喺廣場見面打招呼"><img class="character companion" src="img/chars/${guide}-welcome.png" alt="${guideName}陪小龍打招呼"><img class="character dino" src="img/games/mandarin/dino-${pose}.png" alt="小龍打招呼"></div><div class="greeting-dialogue"><p class="game-instruction">${prompt}</p><p class="game-instruction speech-bubble">「${phrase}」</p><div class="actions"><button id="greeting-listen">${icon('ear')}聽示範</button><button id="greeting-record" class="primary">${icon('microphone')}跟住講 · 錄五秒</button><button id="greeting-stop" hidden>講完啦</button><button id="greeting-play" hidden>聽返自己把聲</button></div><p class="feedback" role="status">錄音只喺呢部機即時回播，離開就清除。</p><div class="actions"><button id="greeting-family">同家長一齊跟讀</button><button id="greeting-next" class="primary" disabled>${state.round===5?'完成打招呼':'下一句'}</button></div></div></div>`;
  setPose(pose);
  ...
  record.onclick=async()=>{
   ...
   let watcher=setInterval(()=>{if(live&&recorder.stream){resume();setPose('listening');status('慢慢講，我哋聽緊。');clearInterval(watcher);}},100);
   try{
   ...
   finally{clearInterval(watcher);busy=false;if(live){setPose(pose);record.disabled=false;stop.hidden=true;resume();}}
  };
