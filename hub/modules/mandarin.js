...
export function mountMandarin(root,state,save,finish,{pause=()=>{},resume=()=>{}}={}){
 let live=true,busy=false,preview=null,previewUrl=null,demo=null;
 const recorder=new LocalRecorder();state.round??=0;
 function clearAudio(){preview?.pause();preview=null;if(previewUrl)URL.revokeObjectURL(previewUrl);previewUrl=null;demo?.pause();demo=null;}
 ...
 function render(){
  ...
  root.querySelector('#greeting-listen').onclick=async()=>{
   if(busy)return;demo?.pause();demo=new Audio(new URL(`../audio/zh-CN/greetings/${id}.mp3`,import.meta.url));
   try{await demo.play();}catch{status('示範聲音未能播放，請家長陪同讀出呢句。');}
  };
  record.onclick=async()=>{
   ...
   try{
    ...
    if(blob?.size){previewUrl=URL.createObjectURL(blob);preview=new Audio(previewUrl);play.hidden=false;status('錄好啦！聽返自己把聲。');}
  ...
  play.onclick=async()=>{if(!preview)return;preview.currentTime=0;preview.onended=()=>{if(live){next.disabled=false;status('多謝你用普通話打招呼！');}};try{await preview.play();}catch{status('未能回播，可以同家長一齊跟讀。');}};
