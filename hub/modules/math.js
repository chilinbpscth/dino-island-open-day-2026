...
export function mountMath(root,state,save,finish){
 state.round??=0;state.recognized??=0;state.served??=[0,0,0];state.attempts??=0;state.usedPieces??=state.served.map(n=>Array.from({length:n},(_,i)=>i));
 ...
 function render(){
  ...
  for(const button of root.querySelectorAll('[data-food]')){
   ...
   const choose=()=>{if(button.disabled)return;selected={index,piece:Number(button.dataset.piece)};root.querySelectorAll('.chosen').forEach(n=>n.classList.remove('chosen'));button.classList.add('chosen');};
  ...
  root.querySelector('#feeding-help')?.addEventListener('click',()=>{const i=intro?state.recognized:order.findIndex((n,i)=>state.served[i]<n);root.querySelector(`[data-food="${fruits[i][0]}"]:not(:disabled)`).classList.add('feeding-hint');feedback(intro?`發光嘅係${fruits[i][1]}。`:`${fruits[i][1]}仲欠 ${order[i]-state.served[i]} ${fruits[i][2]}，逐件餵。`);});
  root.querySelector('#feeding-next')?.addEventListener('click',()=>{if(!live)return;stopVoice();if(state.round===2){live=false;finish();return;}state.round++;state.served=[0,0,0];state.usedPieces=[[],[],[]];state.attempts=0;save();render();listen();});
 }
 ...
 function feed(){
  ...
  const {index:i,piece}=selected,[,name,unit]=fruits[i];
  if(state.usedPieces[i].includes(piece))return;
  if(state.served[i]>=orders[state.round][i]){state.attempts++;save();render();sound('enough');feedback(`${name}已經夠數啦，睇吓另一種仲欠幾多。`);return;}
  state.usedPieces[i].push(piece);state.served[i]++;save();render();sound(complete()?'full':'bite');munch(fruits[i][0]);feedback(complete()?'三種水果都啱啱好，小龍食得開心又舒服！':`${name}：${state.served[i]} ${unit}，要 ${orders[state.round][i]} ${unit}。`);
 }
