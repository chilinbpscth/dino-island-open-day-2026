...
export function mountMotion(root,state,save,finish,lifecycle={},subject='english'){
 const {pause=()=>{},resume=()=>{}}=lifecycle,sequence=sequences[subject],tracker=new PoseActions();state.round??=0;
 let live=true,mode=state.roundDone?'success':'choose',request=0,raf=null,last=null,remaining=5000,holdStart=null,holdPointer=null,commandStarted=false,audio=null;
 const command=()=>sequence[state.round];
 const status=text=>{if(live&&root.querySelector('.feedback'))root.querySelector('.feedback').textContent=text;};
 function stop(){request++;poseCamera.stop();cancelAnimationFrame(raf);raf=null;holdStart=null;holdPointer=null;last=null;audio?.pause();audio=null;}
 function render(){
  root.innerHTML=`<div class="rounds" aria-label="第 ${state.round+1} 回合，共 ${sequence.length} 回合">${sequence.map((_,i)=>`<span class="round-dot ${i<=state.round?'done':''}"></span>`).join('')}</div><h2 class="game-instruction">${subject==='english'?'小龍講，你跟住做':'恐龍蛋守護隊'}</h2><p class="game-instruction">${labels[command()]}</p><div class="motion-stage action-${command()}">${subject==='pe'?'<div class="protected-egg" aria-label="要保護嘅恐龍蛋"></div>':''}<img class="character motion-guide" src="img/chars/${subject==='english'?'dino-baby':'xiaozhi-stretch'}.png" alt="動作示範角色"><span class="motion-cue" aria-hidden="true">${cues[command()]}</span><video class="motion-camera" autoplay muted playsinline ${mode==='camera'||mode==='loading'?'':'hidden'} aria-label="本機鏡像動作畫面"></video></div><p class="feedback" role="status">${mode==='success'?(subject==='pe'?'你幫恐龍蛋安全留喺窩入面！':'你聽指令同小龍一齊郁身，小龍好開心！'):mode==='choose'?'平板放穩，家長陪住，預留郁身位置。':mode==='paused'?'鏡頭同計時已暫停，返嚟再準備。':mode==='family'?'一齊跟示範做，倒數完再長按確認。':'準備鏡頭同本機辨識，請稍等。'}</p><div class="motion-progress" role="progressbar" aria-label="動作進度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><span></span></div><div class="actions">${['choose','paused'].includes(mode)?'<button id="motion-camera-start" class="primary">用鏡頭一齊玩</button>':''}${mode!=='success'&&mode!=='family'?'<button id="motion-family">同家長一齊做</button>':''}${subject==='english'&&mode!=='success'?'<button id="motion-listen">聽指令</button>':''}${mode==='family'?'<span class="motion-countdown" aria-live="polite">5</span><button class="primary" id="motion-hold" disabled>準備一齊做</button>':''}${mode==='success'?`<button id="motion-next" class="primary">${state.round===sequence.length-1?'完成照顧任務':'下一個動作'}</button>`:''}</div>`;
  ...
  root.querySelector('#motion-listen')?.addEventListener('click',()=>{audio?.pause();audio=new Audio(new URL(`../audio/en/actions/${command()}.mp3`,import.meta.url));audio.play().catch(()=>status('指令聲音未能播放，可以跟住圖示同家長一齊做。'));});
  root.querySelector('#motion-next')?.addEventListener('click',()=>{if(!live)return;const wasFamily=state.inputMode==='family';if(state.round===sequence.length-1){live=false;stop();finish();return;}state.round++;state.roundDone=false;save();if(wasFamily)family();else launch();});
 }
 ...
 async function launch(){
  stop();const attempt=request;mode='loading';commandStarted=false;tracker.command=null;tracker.clearAction();pause('motion-setup');pause('motion-transition');render();
  try{
   await poseCamera.start(root.querySelector('video'),(poses,time)=>{
    ...
    if(!commandStarted&&result.status==='ready'){tracker.setCommand(command());commandStarted=true;mode='camera';state.inputMode='camera';save();resume('motion-setup');resume('motion-transition');status('準備好啦，'+labels[command()]+'。');return;}
