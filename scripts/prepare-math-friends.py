...
for name in ('longneck', 'triceratops'):
    source = Image.open(ROOT / f'design/game-assets/drafts/math-friend-{name}-matte.png').convert('RGB')
    ...
    target = ROOT / f'hub/img/games/math/friend-{name}.png'
    output.save(target)
    review = Image.new('RGBA', output.size, '#F6F3EC')
    review.alpha_composite(output)
    review.convert('RGB').save(ROOT / f'design/game-assets/drafts/math-friend-{name}-cream.png')
    print(target)
