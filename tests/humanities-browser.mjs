...
try{
 ...
 assert.match(await page.locator('.expedition-intro').innerText(),/想像遊戲/);await page.locator('#expedition-start').click();await page.waitForFunction(()=>[...document.querySelectorAll('.expedition-landscape')].every(i=>i.complete&&i.naturalWidth));assert.equal(await page.locator('.expedition-landscape').count(),3);await page.screenshot({path:'docs/screenshots/humanities-options-v2.png',fullPage:true});
