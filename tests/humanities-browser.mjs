...
try{
 ...
 assert.match(await page.locator('.expedition-intro').innerText(),/想像遊戲/);await page.locator('#expedition-start').click();
 ...
 await pick('mud');assert.equal((await player()).zones.humanities.game.roundDone,false);await pick('bottle');assert.equal(await page.locator('.expedition-improvements>span').count(),2);await page.locator('#expedition-next').click();
 await page.locator('#back-map').click();await page.waitForURL('**/#/map');await page.goto('http://127.0.0.1:4173/hub/#/play/humanities');assert.equal((await player()).zones.humanities.game.round,2);
 await page.setViewportSize({width:390,height:844});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await pick('forest');assert.equal(await page.locator('.expedition-improvements>span').count(),3);
 await page.setViewportSize({width:1194,height:834});await page.screenshot({path:'docs/screenshots/humanities-v2.png',fullPage:true});assert.equal((await player()).zones.humanities.complete,false);await page.locator('#expedition-next').click();assert.equal((await player()).zones.humanities.complete,true);
