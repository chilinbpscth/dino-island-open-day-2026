...
assert.equal(await page.locator('#certificate-request').inputValue(),firstId,'request ID remains visible for recovery after reload');
await expect(page.locator('#certificate-link')).toBeVisible();
assert.match(await page.locator('#certificate-link').getAttribute('href'),/SAVED_DEVICE\/exec\?view=certificate&token=/);
