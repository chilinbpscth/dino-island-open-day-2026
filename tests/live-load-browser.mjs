...
try{
 ...
 await page.evaluate(()=>{window.failPreparation=false;window.calls=[];window.records.outsider={id:'outside-test',zonesCompleted:10,totalMs:1};});await page.locator('#run').click();await expect(page.locator('#status')).toHaveText('preparation-failed');assert(await page.evaluate(()=>window.calls.every(c=>c.method!=='uploadCertificate')));assert((await page.locator('#result').innerText()).includes('現有名次會遮住測試紀錄'));
 console.log('PASS load harness: explicit start, ten concurrent clients, credential-free public reads, private report, reused batch IDs, upload/preparation failures and occupied ranking guard');
