...
export function loadBatch(id=crypto.randomUUID()){
 if(!/^[0-9a-f-]{36}$/.test(id))throw Error('批次 ID 格式不正確');
 return {batchId:id,clients:Array.from({length:10},(_,i)=>({id:`load-${id}-${i+1}`,requestId:`load-cert-${id}-${i+1}`,name:`負載測試${String(i+1).padStart(2,'0')}`,version:2,status:'pending'}))};
}
export async function runLoad(report,rpcs,publicRpc,image,endpoint,update=()=>{}){
 const zones=n=>Object.fromEntries(ZONES.slice(0,n).map(id=>[id,1000]));
 ...
 try{await publicRpc('getBoard');}catch(e){report.phase='preparation-failed';report.pollErrors.push(e.message);update(report);return report;}
 const started=performance.now();report.phase='running';update(report);
 const poll=async()=>{
  while(performance.now()-started<60000){
   try{const board=await publicRpc('getBoard');for(const client of report.clients){const row=board.players.find(p=>p.id===client.id);if(row&&row.zonesCompleted===10&&row.totalMs===10000&&client.boardMs===undefined)client.boardMs=Math.round(performance.now()-started);}}catch(e){report.pollErrors.push(e.message);}
