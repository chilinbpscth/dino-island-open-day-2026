...
const feed=async fruit=>{await page.locator(`[data-food="${fruit}"]:not(:disabled)`).first().click();await page.locator('[data-dino]').click();};
try{
 ...
 await page.locator('[data-food="banana"]:not(:disabled)').first().click();assert.equal((await player()).zones.math.game.recognized,0);
 ...
 assert.deepEqual((await player()).zones.math.game.served,[0,0,1]);await page.locator('#feeding-help').click();await expect(page.locator('[data-food="apple"]:not(:disabled)').first()).toHaveClass(/feeding-hint/);
