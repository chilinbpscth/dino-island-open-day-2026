...
try{
 ...
 for(const fruit of ['apple','banana','strawberry'])await page.locator(`[data-food="${fruit}"]`).tap();
 const touch=await context.newCDPSession(page);
 async function drag(fruit,cancel=false){const from=page.locator(`[data-food="${fruit}"]`);await from.scrollIntoViewIfNeeded();const a=await from.boundingBox(),b=await page.locator('[data-dino]').boundingBox(),start={x:a.x+a.width/2,y:a.y+a.height/2},end={x:b.x+b.width/2,y:b.y+b.height/2};await touch.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{...start,id:1}]});for(let i=1;i<=8;i++)await touch.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x:start.x+(end.x-start.x)*i/8,y:start.y+(end.y-start.y)*i/8,id:1}]});await touch.send('Input.dispatchTouchEvent',{type:cancel?'touchCancel':'touchEnd',touchPoints:[]});}
