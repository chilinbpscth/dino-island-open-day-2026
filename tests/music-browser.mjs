...
try{
 ...
 assert.equal(await page.locator('.band-celebrating').count(),1);assert.match(await page.locator('#game').innerText(),/奏出 4 拍啦/);assert.doesNotMatch(await page.locator('#game').innerText(),/再輪到你/);await page.emulateMedia({reducedMotion:'reduce'});assert.equal(await page.locator('.band-player .character').first().evaluate(n=>getComputedStyle(n).animationName),'none');await page.emulateMedia({reducedMotion:'no-preference'});await page.reload();await page.locator('#band-next').waitFor();assert.equal((await player()).zones.music.complete,false);
 await page.setViewportSize({width:390,height:844});await page.screenshot({path:'docs/screenshots/music-mobile-v2.png',fullPage:true,animations:'disabled'});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await page.setViewportSize({width:1194,height:834});await page.screenshot({path:'docs/screenshots/music-v2.png',fullPage:true});await page.locator('#band-next').click();assert.equal((await player()).zones.music.complete,true);assert.deepEqual(errors,[]);
